...
$thispic = $_GET['record'];
    ...
    $query = "SELECT * FROM photo WHERE cnumber='" . $thispic . "'";
...
echo '    <link rel="unapi-server" type="application/xml" title="unAPI" href="http://photogrammar.yale.edu/records/unapi.php">';
echo '<abbr class="unapi-id" title="' . $_GET["record"] . '"></abbr>';
echo <<<_END
...
/*
    <link rel="unapi-server" type="application/xml" title="unAPI" href="http://photogrammar.yale.edu//records/unapi.php">
...
if(isset($_GET['record']))
{
    ...
    $query = "SELECT * FROM photo WHERE cnumber='" . get_post('record'). "'";
    //echo $query;
    //echo '<br />';
...
//    echo '<div id="record-repository" class="record-field"><h3 class="record-heading">Repository</h3><span class="record-text">Library of Congress Prints and Photographs Division Washington, D.C. 20540 USA http://hdl.loc.gov/loc.pnp/pp.print</span></div><!--/#record-repository-->';
    echo '<div id="record-digitalid" class="record-field"><h3 class="record-heading">Call Number<span style="font-size:.8em;color:grey; font-weight:normal;"> (Library of Congress)</span></h3><a class="record-text" target="_blank" href="http://www.loc.gov/pictures/item/' . $_GET['record'] . '">' . mysql_result($result, 0, 'cnumber2') . '</a></div>';

   $similarquery = 'select similarphoto, pname, year, howsimilar, title, thumb_url from similarity_new JOIN photo ON photo.cnumber=CONCAT(similarity_new.similarphoto, "/PP") where thisphoto="' . substr($_GET['record'],0,13) . '" ORDER BY howsimilar DESC LIMIT 5;';

   $similarresult = mysql_query($similarquery);
   if ((mysql_num_rows($similarresult)) > 0) {
      echo '<div id="record-similar" class="record-field"><h3 class="record-heading">Similar Photos</h3>';
	  echo '<table style="line-height:100%;">';
	   while($row = mysql_fetch_array($similarresult)) {


	   		echo '<tr><td style="border-bottom:1px dotted grey;padding-bottom:5px;"><span style="font-size:.8em;font-weight:bold;">' . $row['pname'] . ', ' . $row['year'] . '</span> <span style="font-size:.8em;color:LightGrey;"> (' . (round($row['howsimilar'],2)*100) . '%)</span><br><a target="_blank" href="/records/index.php?record=' . $row['similarphoto'] . '/PP"><img src="http://photogrammar.research.yale.edu/photos' . $row['thumb_url'] . '" onload="this.width/=2;this.onload=null;"  style="float:left;margin-right:5px;" /></a><a class="record-text" target="_blank" href="/records/index.php?record=' . $row['similarphoto'] . '/PP">'. $row['title'] . '</a></td></tr>';

	   		};
	   	echo '</table></div><!--/#record-digitalid-->';
    };
    echo '</div><!--/#record-meta-->';
    echo '<div id="record-image">';
    echo '<a href="http://photogrammar.research.yale.edu/photos' . mysql_result($result, 0, 'large_url') . '">';
    echo '<img style="max-height:500px" src="';
    $photourl = mysql_result($result, 0, 'large_url');

  	  	echo 'http://photogrammar.research.yale.edu/photos' . mysql_result($result, 0, 'large_url');


    echo '"/></a></div><!--/#record-image-->';
