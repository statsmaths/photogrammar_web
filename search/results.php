...
mysql_select_db($db_database) or die('Unable to connect to MySQL.');
...
if(isset($_GET['start']))
{
    ...
    $fval['van'] = $_GET['van'];
    $fval['lot'] = $_GET['lot'];
    if ($_GET['city'] != "NA") {$fval['city'] = $_GET['city']; $city=$_GET['city'];}
    if ($_GET['city'] == "NA") {$fval['city'] = ''; $city = '';}
    ...
    $query = "SELECT * FROM photo WHERE fips != 'NA' AND pname LIKE '%" . get_post('pname') . "%' ";
/*              "lotnum LIKE '%" . get_post('lot') . "%' AND " . */
  if ($_GET['lot']!=""){     $query .=         "AND lotnum ='" . get_post('lot') . "' ";};
  $query .= "AND city LIKE '%" . $city . "%' AND " .
...
/*     echo '<span style="font-family:monospace;font-size:0.7em;color:grey;">' . $query . '</span><br>'; */
    if (!$result) die ("Database access failed. Possibly bad url.");
...
*/
    ...
    for($j = get_post('start'); $j < min(get_post('start') + 60, $rows); ++$j)
    {
       ...
       echo '<div class="results-image"><a href=/records/index.php?record=' .mysql_result($result, $j, 'cnumber') . '>';
    ...
    if (substr(mysql_result($result, $j, 'thumb_url'), -2) != 'NA') {
      ...
      if (mysql_result($result, $j, 'thumb_url') != '') {
        echo 'http://photogrammar.research.yale.edu/photos' . mysql_result($result, $j, 'thumb_url');
      }
    }
    if (substr(mysql_result($result, $j, 'thumb_url'), -2) == 'NA') {
      echo 'http://photogrammar.research.yale.edu/photos' . mysql_result($result, $j, 'small_url');
    }
       echo '" />';
